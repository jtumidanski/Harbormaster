...
argument-hint: Brief description of the feature idea (e.g., "bucket replication dashboard", "iam policy editor")
---
...
1. Read project docs in `docs/` and any relevant service docs in `services/*/docs/` that the idea touches. (These may not exist yet for an early task — that's fine, note it.)
2. Check `docs/TODO.md` for related items if present.
3. Scan existing task folder names (don't read full PRDs unless one looks directly related).
4. Identify which services would be affected (or, for the very first tasks, which services need to be created).
...
5. **Affected services** (existing or to-be-created)
...
[Which services are affected (or need to be created) and what changes in each]

## 8. Non-Functional Requirements

[Performance, security, observability considerations]
...
- Respect existing architectural patterns from project/service docs and the codebase (once they exist).
- API designs follow JSON:API conventions where applicable.
