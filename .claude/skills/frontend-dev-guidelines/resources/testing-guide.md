...
Harbormaster UI uses **Jest 30** with **React Testing Library** and **jsdom** environment. Tests live alongside source files in `__tests__/` directories or as `*.test.{ts,tsx}` files.
...
```typescript
// components/features/buckets/__tests__/CreateBucketDialog.test.tsx
...
describe('CreateBucketDialog', () => {
  const mockOptions = [
    { id: 'r1', attributes: { region: 'us-east-1' } },
    { id: 'r2', attributes: { region: 'eu-west-1' } },
  ];

  beforeEach(() => {
    jest.clearAllMocks();
    regionsService.getRegionOptions.mockResolvedValue(mockOptions);
  });

  it('fetches options when dialog opens', async () => {
    render(<CreateBucketDialog open onOpenChange={jest.fn()} />);
    await waitFor(() => {
      expect(regionsService.getRegionOptions).toHaveBeenCalled();
    });
  });

  it('shows error when options fail to load', async () => {
    regionsService.getRegionOptions.mockRejectedValue(new Error('Network error'));
    render(<CreateBucketDialog open onOpenChange={jest.fn()} />);
    await waitFor(() => {
  ...
  it('allows typing in name field', async () => {
    render(<CreateBucketDialog open onOpenChange={jest.fn()} />);
    await waitFor(() => expect(regionsService.getRegionOptions).toHaveBeenCalled());
    const input = screen.getByPlaceholderText(/enter bucket name/i);
    await userEvent.type(input, 'my-bucket');
    expect(input).toHaveValue('my-bucket');
  });

  it('does not render when closed', () => {
    render(<CreateBucketDialog open={false} onOpenChange={jest.fn()} />);
    expect(screen.queryByText('Create New Bucket')).not.toBeInTheDocument();
  });
});
```
