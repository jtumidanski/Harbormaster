...
When scaffolding a new Harbormaster service, complete ALL of these steps. Do not skip any.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: <service-name>
        image: ghcr.io/<owner>/harbormaster-<service-name>:latest
        ports:
        - containerPort: 8080
        env:
        - name: DB_HOST
...
2. `/backend-audit` — audits against Harbormaster backend developer guidelines
